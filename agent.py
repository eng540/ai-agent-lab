...
SYSTEM_INSTRUCTION = """
You are AI Agent Lab, an execution-oriented agent. Plan implicitly, inspect relevant files, use tools, verify important outputs, and report only actions actually completed.
Rules: never pretend an action happened; only use provided workspace tools; inspect before conclusions; after important writes, read back the file; keep execution focused.
"""
...
def run_agent(message: str, event_callback=None) -> str:
    ...
    try:
        emit("planning", "تحليل المهمة واختيار الأدوات")
        interaction = client.interactions.create(model=MODEL_ID, input=message, tools=TOOLS, system_instruction=SYSTEM_INSTRUCTION, generation_config={"thinking_level": "medium", "temperature": 0.2, "max_output_tokens": 8192})
        ...
        for round_number in range(1, MAX_TOOL_ROUNDS + 1):
            ...
            results = []
            for call in calls:
                emit("tool_requested", call.name)
                try:
                    result = _execute_tool(call.name, call.arguments or {})
                    payload = {"ok": True, "result": result}
                    emit("tool_success", call.name)
                except Exception as exc:
                    payload = {"ok": False, "error": str(exc)}
                    emit("tool_failed", f"{call.name}: {exc}")
                results.append({"type": "function_result", "name": call.name, "call_id": call.id, "result": [{"type": "text", "text": json.dumps(payload, ensure_ascii=False)}]})

            emit("round_completed", f"جولة الأدوات {round_number}")
            interaction = client.interactions.create(model=MODEL_ID, previous_interaction_id=interaction.id, input=results, tools=TOOLS, system_instruction=SYSTEM_INSTRUCTION, generation_config={"thinking_level": "medium", "temperature": 0.2, "max_output_tokens": 8192})

        emit("limit_reached", "تم بلوغ الحد الآمن لجولات الأدوات")
